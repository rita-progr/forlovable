  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { setStatus, setSubmitting }) => {
      ...
      try {
        ...
        await register(values.email, values.password, values.changepassword, values.full_name);
        
        // После успешной регистрации перенаправляем на страницу логина
        navigate('/auth/login', {
          state: {
            message: 'Registration successful. Please login to continue.',
            email: values.email // Опционально: можно передать email для автозаполнения
          }
        });
      } catch (error: any) {
        console.error(error);
